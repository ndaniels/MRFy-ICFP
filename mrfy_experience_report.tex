...
% 
\keywords
memoization, stochastic search, QuickCheck, homology detection
...
\remark{The opening sentence here sounds awkward, but I'm not sure if it's
  because I'm reading it wrong. It seems like ``technique'' should be
  pluralized, or perhaps ``would use now-classic: a lazy and modular
  techniqe''.}
...
\mrfy\ can compute an alignment to a complex, 12-stranded ``beta sandwich''
structure within one minute, while on the same structure, SMURF allocates
over 16GB of memory and does not terminate even after eight ours.
The Markov random field model for this structure contains 252 nodes,
...
11~amino acids.
On these smaller problems, performance is good enough that \mrfy\ can
do its job.

We benchmarked \mrfy\ using a model of an ``8-bladed beta propeller,''
...
Noah Daniels and Andrew Gallant were funded in part by NIH grant 1R01GM080330 (to Lenore Cowen).

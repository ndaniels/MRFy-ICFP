...
\remark{we promised a better name for HMMState. Thoughts?}
\begin{smallverbatim}
...
encountered up to this point:\remark{we promised to characterize the missing cases}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
other researchers may have to pay as well.
...
was useful.\remark{I hate this ``explanation.'' ---NR}
...
also the \emph{age} of the \texttt{younger} point which is
moved~to.\remark{This word ``age'' is \emph{entirely} bogus!}
...
that results in a worse score.\remark{And I hate this explanation
too.} 
...
tag each one with its age and utility.\remark{There's that bogus age
again.}
...
\noindent
We have implemented two kinds of stop functions...



N.B. WE HAVE TWO KINDS OF NON-MODULARITY.  FIRST, TERMINATION HAS TO
LOOK AT \emph{ALL} STATES, EVEN THOUGH WE WOULD PREFER THAT IT LOOK
ONLY AT USEFUL STATES.  SECOND, AGE IS EVERYWHERE.  IN PARTICULAR, WE
CANNOT SEPARATE THE AGE OF THE CURRENT STATE FROM THE UTILITY
DECISION.



\remark{We need to explain why we don't just generate an infinite lazy
list of thingies.  And perhaps we can consider an infinite tree!}
...
(Hill climbing accepts a point only if it scores better than its
predecessor;
...
\fi
\end{itemize}
This \texttt{SearchStrategy} interface somewhat resembles an object-oriented
...
\smallfuzzverbatiminput{11pt}{orig-search}
...
hardware, with between 12 and 16 cores. We have access to only
one 48-core system, and while MRFy will keep all 48 cores busy, it
performs better when given no more than 16 cores.\remark{give specific
numbers here}
...
in \figref{alignment}.
It~comprises about 12,000 lines of~C++.\remark{reviewers seemed to want a
table of this information, but that seems like a huge waste of space.}
...
in 30~seconds.\remark{reconcile this vague paragraph with performance section above}
This performance
...
An~anonymous referee\remark{Just one referee?} spurred us to think

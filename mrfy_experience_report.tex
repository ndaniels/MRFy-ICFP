...
% 
\keywords
memoization, stochastic search, parallel strategies,  QuickCheck, remote homology detection
...
If~$x'$ is located in position~$i'$ in the query sequence, Viterbi's
equations must be altered so that, for example
$V_{j}^{\prime M}(i)$ depends not only on
$V_{j-1}^{\prime M}(i-1)$ but also on
$V_{\pairedwith j}^{\prime M}(i')$.\remark{I can't get an equation I
...
\remark{given that this experience is no longers as relevant,
and we're tight on space, should we cut this entire paragraph
about profiling? --ND}

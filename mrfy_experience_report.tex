...
computes an infinite sequence of generations, and another function
uses a finite prefix to decide on an approximation.
But making MRFy's search lazy and modular search is not so easy.
 because MRFy's search is stochastic.

In~a stochastic search, \emph{movement} between points in the search
space is not the same as \emph{progress}.
For example, suppose you are searching for the top of a hill
and you take a small jump in a random direction.
If~you arrive at a higher point than your point of origin, you have
made progress.
Otherwise you have just moved.
\smallverbatiminput{movequality}
We~hoped to implement \mrfy's search by composing functions:
\begin{itemize}
\item
From a given point, a \emph{generator} produces an infinite sequence
of moves.
\item
From such a sequence, a \emph{classifier} chooses the first move that
represents progress.
What sort of move constitutes progress is not only different for
different kinds of searches, but may also be different at different
\emph{times} in a search.
\end{itemize}
A~generator and classifier can be composed to produce a sequence of
progressing generations, and a test function can terminate the search
after examining a finite
prefix.
But this sketch contains a fatal flaw:
a generator may produce an infinite sequence of generations without any
progress.
And then, when the test function tries to examine a finite prefix of
progressing generations, it will diverge.
We~might conclude that
\mrfy's search algorithm is inherently non-modular:
production of progressing generations cannot be decoupled from the
...
N.B.~The following transition no longer makes any sense:
\endgroup
We~pass the

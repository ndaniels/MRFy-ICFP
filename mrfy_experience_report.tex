...
\remark{Problem: the subscripts are not so easy to read, and notation
is not quite consistent with \figref{viterbi}, definition of $a'$ at
bottom right.}
\begin{itemize} 
\item
...
\begin{smallverbatim}
data TProb =   --- THIS DEFN IS A DAMN LIE
...
\begin{smallverbatim}
data TProbs =  -- MORE DAMN LIES
     TProbs { m_m :: TProb, m_i :: TProb, m_d :: TProb
            , i_m :: TProb, i_i :: TProb
            , d_m :: TProb, d_d :: TProb }
\end{smallverbatim}
In~our code, we select from this 7-tuple using the \texttt{edge}
function:\remark{Broken!  This function no longer appears in our examples}
...
THIS PART IS BROKEN.  THE A ARRAY NEEDS TWO SUBSCRIPTS, AND THE E
NOTATION IS INCONSISTENT (HAS MAGICALLY CHANGED FROM BRACKETS TO SUBSCRIPTS).
...
the score.\remark{Changes in the code appear to have
made \texttt{fmap} irrelevant}
...
In this code, EXPLAIN EXTEND, EXPLAIN ESCORE, EXPLAIN
ASCORE. 
we~use~\texttt{/+/} to add transition and emission scores,
and we use \texttt{fmap} to add \texttt{Mat} to the list of states
encountered up to this point:\remark{BROKEN! \texttt{fmap} is gone}
...
Becayuse $W_j^M(i)$~depends not only on $x_i$ and its local neighbors but

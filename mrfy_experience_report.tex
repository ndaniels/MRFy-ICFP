...
\remark{I thought the query sequence was an amino-acid sequence.}
...
structures.
The software is ``trained'' \remark{do we want to talk ``Training''?}
...
\emph{match state}.
\remark{Time to introduce the elements of Viterbi's equations!}
...
% time for types.
% first, ideal: amino acids as an algebraic DT
% next, back off this: we're working with an existing input format
% so, how is the graph represented (vector of state-transitions, 
% vector of emissions)
% so show those types
...
But because real proteins are three-dimensional structures,
their amino-acid sequences are subject to \emph{nonlocal}
constraints. 
In~particular, when a real protein folds, groups of amino acids
that are far away in the one-dimensional sequence can nevertheless be
adjacent in three-dimensional space.
Moreover, two such groups can become hydrogen-bonded to one another,
effectively ``stuck together.''
Such groups are called \emph{beta strands}.
\remark{I hope this is relatively accurate. --NR}
...
of homologous proteins?]}
 ...
 simply implements the Viterbi algorithm for computing the optimal parse 
...
\remark{I don't like this. I really struggled with this section. ---AG}

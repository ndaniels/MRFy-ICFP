...
\remark{This looks wrong.  $x$ should be subscripted with a
query-sequence index, but $\pairedwith j$ is a model index.}
...
If~you look carefully at how \texttt{successors} is computed,
we~hope you will understand why we
...
Simulated annealing reuses hill climbing's
 \texttt{nextPt} function.
Simulated annealing's  new \texttt{utility} function took ten
...
We note that the \texttt{SearchStop} functions are not as composable as we
would like, and we would like to have a combinator library for searches.
...
We~also found an opportunity to improve performance through parallelism.
...
We have access to a
one 48-core, 2.3GHz AMD Opteron 6176 system, on which we benchmarked \mrfy\ performing
a search on a large protein structure (an ``8-bladed beta-propeller'')
comprising 343 hidden Markov model nodes and 40 beta strands, and a query sequence
592 amino acids long.
Figure~\ref{multicore} illustrates the mean speedup \mrfy\ achieves as the number of
processing cores increases.
Optimal speedup appears to occur between 12 and 16 processors.
...
SMURF.
Both \mrfy\ and SMURF, when provided with a Markov random field containing no beta strands,
simply compute the optimal solution using Viterbi's algorithm.
For both \mrfy\ and SMURF, this task admits no parallelism.
On a 2.6GHz Intel Core i7,
given a micro-benchmark consisting of a contrived query sequence
of 2000 amino acids (which represents a large protein),
and a hidden Markov model of 343 nodes and no beta strands,
\mrfy\ takes 2.32 seconds to compute a Viterbi solution.
SMURF performs the same computation in 0.29 seconds.

But unlike SMURF, \mrfy\ doesn't have to run on large models.
For example, on the ``8-bladed propeller'' benchmark, the beta strands
break the Markov random field into 41 pieces, each of which is much smaller
than the original (typically 10 or fewer nodes rather than 343).
And, each \texttt{placement} breaks the query sequence into 41 pieces, each of
which is much smaller than the original (typically 20 or fewer amino acids
rather than 592).
Thus, \mrfy\ repeatedly runs Viterbi's algorithm on 
a~great many smaller models, and the independence of these models
means that \mrfy\ can execute them in parallel.
...
Clearly, micro-benchmarks do not tell the whole story.
Our goal is to solve a biological problem, and though our implementation of
Viterbi's algorithm is comparatively slow, we can still solve a biological problem
quickly.

...
\fi
the software that incorporates this new
code 
...
in~addition to computing the probability of the most likely path,
\mrfy\ also computes the most likely path (sequence of states).
...
\remark{Question ``why not an infinite list'' has gone missing.}
...
\remark{This part is gross and still needs work!}
...
on a large\remark{QUANTIFY!!} protein structure, than when given 16 cores, which takes an average
of 37 seconds. 
...
but his primary contributions to MRFy have been limited to \emph{post hoc}
...
\remark{Where is Andrew's shitty paragraph?  It shoudl replace this one?}
...
Koen Claessen instantly diagnosed an inappropriately strict monad.

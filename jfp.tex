...
atoms, and so within the protein, what's left of the amino acid is called
a \emph{residue}.
In~this paper we use the terms ``amino acid'' and ``residue''
interchangeably.
}
...
But~seeing \mrfy\ solve some problems 8~times slower then
SMURF felt like a poor showing for Haskell.
...
\item
From
the \texttt{next} function, we~got the idea that Viterbi's algorithm
...
to GHC's profiler, running Viterbi's algorithm on a single query sequence
of 592~residues, against a
model of 343~nodes.
To~help you understand the results, we~observe that the memo table
for \verb+vee''+ has 610,197 entries: one for each combination of
$\hat s$, $j$, and~$i$.
The~profiler told us that a lot of time was spent in memory
management, and that \mrfy\ allocated 531MB, or 7.85KB
per~entry.
This seems excessive.
%
The profile contained some information we
...
To~try to understand and eliminate the unwanted allocations,
we~explored a many variations on the code, of~which the most ambitious (and
most tedious) was to desugar the list comprehension by~hand and to
replace the standard list operations with operations that participate
...
to implement.
TALK ABOUT RICHARD SENINGTON'S STUFF HERE.

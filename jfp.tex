...
%%
In~our representation, each state is referred to by its position~$j$
within the alignment and by one of these labels:
...
Moreover, in~order to work with positive numbers, 
...
V_{j}^{\prime \hat s}(i) = e^{\prime}_{\hat s_{j}}(x_{i}) 
  + \min\limits_s \left\{
       \vsum{V_{\mathit{pnode}\;\hat s\;j}^{\prime
       s}(\mathit{presidue}\;\hat s\;i)} {a^{\prime}_{s \,\hat
...
We also combined Viterbi's three equations into one.
Each equation has the same form, but is parameterize over three
values determined by the right-hand state~$\hat s$:
the~index of the previous node ($j$~or~$j-1$),
the~index of the previous residue ($i$~or~$i-1$),
and the set of previous states~$s$ over which the minimum is taken.
If~we abstract the index computations into functions
$\mathit{pnode}$ 
and 
$\mathit{presidue}$
respectively,
we~get the combined, abstract form of Viterbi's equations shown in
\figref{viterbi-abstracted}.
The~implementation of \figref{viterbi-abstracted} looks like this:
\smallverbatiminput{hoviterbi}
Function \texttt{next} makes the recursive call to 
